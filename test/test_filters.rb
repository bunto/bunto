...
class TestFilters < BuntoUnitTest
  class BuntoFilter
    include Bunto::Filters
    attr_accessor :site, :context

    def initialize(opts = {})
      @site = Bunto::Site.new(Bunto.configuration(opts.merge('skip_config_files' => true)))
  ...
  context "filters" do
    setup do
      @filter = BuntoFilter.new({"source" => source_dir, "destination" => dest_dir, "timezone" => "UTC"})

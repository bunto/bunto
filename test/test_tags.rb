...
class TestTags < BuntoUnitTest

  def setup
    FileUtils.mkdir_p("tmp")
  end

  def create_post(content, override = {}, converter_class = Bunto::Converters::Markdown)
...
CONTENT
  ...
  def highlight_block_with_opts(options_string)
    Bunto::Tags::HighlightBlock.parse('highlight', options_string, ["test", "{% endhighlight %}", "\n"], {})
  end

  context "language name" do
    should "match only the required set of chars" do
      r = Bunto::Tags::HighlightBlock::SYNTAX
      assert_match r, "ruby"
      assert_match r, "c#"
  ...
  context "with the pygments highlighter" do
    ...
    context "post content has highlight with file reference" do
      setup do
        fill_post("./bunto.gemspec", {'highlighter' => 'pygments'})
      end

      should "not embed the file" do
        assert_match %{<pre><code class="language-text" data-lang="text">./bunto.gemspec</code></pre>}, @result
      end
    end

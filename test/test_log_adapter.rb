...
class TestLogAdapter < BuntoUnitTest
  class LoggerDouble
    attr_accessor :level
  ...
  context "#log_level=" do
    should "set the writers logging level" do
      subject = Bunto::LogAdapter.new(LoggerDouble.new)
      subject.log_level = :error
      assert_equal Bunto::LogAdapter::LOG_LEVELS[:error], subject.writer.level
    end
  end
  ...
  context "#adjust_verbosity" do
    ...
    should "set the writers logging level to debug when verbose" do
      ...
      assert_equal Bunto::LogAdapter::LOG_LEVELS[:debug], subject.writer.level
    end
    ...
    should "not change the writer's logging level when neither verbose or quiet" do
      ...
      refute_equal Bunto::LogAdapter::LOG_LEVELS[:error], subject.writer.level
      refute_equal Bunto::LogAdapter::LOG_LEVELS[:debug], subject.writer.level
    ...
    should "call #debug on writer return true" do
      ...
      logger = Bunto::LogAdapter.new(writer)
  ...
  context "#abort_with" do
    should "call #error and abort" do
      logger = Bunto::LogAdapter.new(LoggerDouble.new)
  ...
  context "#messages" do
    should "return an array" do
      assert_equal [], Bunto::LogAdapter.new(LoggerDouble.new).messages
    end

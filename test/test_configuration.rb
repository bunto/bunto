...
class TestConfiguration < BuntoUnitTest
  @@defaults = Bunto::Configuration::DEFAULTS.add_default_collections.freeze
  ...
  context "loading configuration" do
    ...
    should "fire warning with no _config.yml" do
      ...
      assert_equal @@defaults, Bunto.configuration({})
    end
  ...
  context "loading config from external file" do
    ...
    should "load different config if specified" do
      ...
      assert_equal Utils.deep_merge_hashes(@@defaults, { "baseurl" => "http://wahoo.dev" }), Bunto.configuration({ "config" => @paths[:other] })
    end

    should "load default config if path passed is empty" do
      allow(SafeYAML).to receive(:load_file).with(@paths[:default]).and_return({})
      allow($stdout).to receive(:puts).with("Configuration file: #{@paths[:default]}")
      assert_equal @@defaults, Bunto.configuration({ "config" => @paths[:empty] })
    end

    should "successfully load a TOML file" do
      Bunto.logger.log_level = :warn
      assert_equal @@defaults.clone.merge({ "baseurl" => "/you-beautiful-blog-you", "title" => "My magnificent site, wut" }), Bunto.configuration({ "config" => [@paths[:toml]] })
      Bunto.logger.log_level = :info
    end
    ...
    should "load multiple config files" do
      ...
      assert_equal @@defaults, Bunto.configuration({ "config" => [@paths[:default], @paths[:other], @paths[:toml]] })
    end
    ...
    should "load multiple config files and last config should win" do
      ...
      allow($stdout).to receive(:puts).with("Configuration file: #{@paths[:default]}")
      allow($stdout).to receive(:puts).with("Configuration file: #{@paths[:other]}")
      assert_equal Utils.deep_merge_hashes(@@defaults, { "baseurl" => "http://wahoo.dev" }), Bunto.configuration({ "config" => [@paths[:default], @paths[:other]] })
    end
  end
end

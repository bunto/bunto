...
class TestDocument < BuntoUnitTest
  ...
  context "a document in a collection" do
    ...
    should "know its data" do
      assert_equal "Bunto.configuration", @document.data["title"]
    ...
    should "be jsonify-able" do
      ...
      assert_equal "Bunto.configuration", parsed["title"]
      ...
      assert_equal "Bunto.escape", next_doc["title"]

      next_prev_doc = next_doc["previous"]
      assert_equal "Bunto.configuration", next_prev_doc["title"]
      ...
      assert_equal "Bunto.sanitized_path", next_next_doc["title"]
  ...
  context "documents in a collection with custom title permalinks" do
    ...
    should "produce the right URL if they have a slug" do
      assert_equal "/slides/so-what-is-bunto-exactly", @document.url
    end

    should "produce the right destination file if they have a slug" do
      dest_file = dest_dir("slides/so-what-is-bunto-exactly.html")
      assert_equal dest_file, @document.destination(dest_dir)
    end
  ...
  context "a static file in a collection" do
    setup do
      ...
      @document = @site.collections["slides"].files.find do |doc|
        doc.relative_path == "_slides/octobunto.png"
      end
      @dest_file = dest_dir("slides/octobunto.png")
    end

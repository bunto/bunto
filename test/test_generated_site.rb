...
class TestGeneratedSite < BuntoUnitTest
...
OUTPUT
  ...
  context "generating limited posts" do
    setup do
      clear_dest
      config = Bunto::Configuration::DEFAULTS.merge({'source' => source_dir, 'destination' => dest_dir, 'limit_posts' => 5})
      @site = fixture_site(config)
      @site.process
      @index = File.read(dest_dir('index.html'))
    end
    ...
    should "ensure limit posts is 0 or more" do
      assert_raises ArgumentError do
        clear_dest
        config = Bunto::Configuration::DEFAULTS.merge({
          'source' => source_dir,

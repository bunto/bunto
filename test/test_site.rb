...
class TestSite < BuntoUnitTest
  context "configuring sites" do
    should "have an array for plugins by default" do
      site = Site.new(Bunto::Configuration::DEFAULTS)
    ...
    should "have an array for plugins if passed as a string" do
      site = Site.new(Bunto::Configuration::DEFAULTS.merge({'plugins_dir' => '/tmp/plugins'}))
      assert_equal ['/tmp/plugins'], site.plugins
    end

    should "have an array for plugins if passed as an array" do
      site = Site.new(Bunto::Configuration::DEFAULTS.merge({'plugins_dir' => ['/tmp/plugins', '/tmp/otherplugins']}))
      assert_equal ['/tmp/plugins', '/tmp/otherplugins'], site.plugins
    end

    should "have an empty array for plugins if nothing is passed" do
      site = Site.new(Bunto::Configuration::DEFAULTS.merge({'plugins_dir' => []}))
      assert_equal [], site.plugins
    end

    should "have an empty array for plugins if nil is passed" do
      site = Site.new(Bunto::Configuration::DEFAULTS.merge({'plugins_dir' => nil}))
      assert_equal [], site.plugins
    end

    should "expose default baseurl" do
      site = Site.new(Bunto::Configuration::DEFAULTS)
      assert_equal Bunto::Configuration::DEFAULTS['baseurl'], site.baseurl
    end

    should "expose baseurl passed in from config" do
      site = Site.new(Bunto::Configuration::DEFAULTS.merge({'baseurl' => '/blog'}))
  ...
  context "creating sites" do
    ...
    should "expose bunto version to site payload" do
      assert_equal Bunto::VERSION, @site.site_payload['bunto']['version']
    end
    ...
    context 'using a non-default markdown processor in the configuration' do
      should 'use the non-default markdown processor' do
        class Bunto::Converters::Markdown::CustomMarkdown
          def initialize(*args)
            @args = args
          end
        ...
        Bunto::Converters::Markdown.send(:remove_const, :CustomMarkdown)
      end

      should 'ignore, if there are any bad characters in the class name' do
        module Bunto::Converters::Markdown::Custom
          class Markdown
            def initialize(*args)
              @args = args
            end
        ...
        # Do some cleanup, we don't like straggling stuff's.
        Bunto::Converters::Markdown.send(:remove_const, :Custom)
      end
    end
    ...
    context "manipulating the Bunto environment" do
      setup do
        @site = Site.new(site_configuration({
          'incremental' => false
        }))
      ...
      context "in production" do
        ...
        teardown do
          ENV.delete("BUNTO_ENV")
        end

        should "be overridden by BUNTO_ENV" do

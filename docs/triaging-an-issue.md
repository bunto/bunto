...
request or a bug. For the Bunto project the following definitions are used
to identify a feature or a bug:

**Feature** - A feature is defined as a request that adds functionality to
Bunto outside of its current capabilities.
**Bug** - A bug is defined as an issue that identifies an error that a user
(or users) encounter when using current Bunto functionalities.
...
@buntobot will automatically mark issues as `stale` if no activity occurs for at least one month. @buntobot leaves a comment asking for information about reproducibility in current versions. If no one responds after another month, the issue is automatically closed.

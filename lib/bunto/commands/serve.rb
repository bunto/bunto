    ...
    class Serve < Command
      ...
      class << self
        ...
        def process(options)
          ...
          Bunto.logger.info "Server address:", server_address(s, options)

          if options['detach'] # detach the server
            pid = Process.fork { s.start }
            Process.detach(pid)
            Bunto.logger.info "Server detached with pid '#{pid}'.", "Run `pkill -f bunto' or `kill -9 #{pid}' to stop the server."
        ...
        def start_callback(detached)
          unless detached
            Proc.new { Bunto.logger.info "Server running...", "press ctrl-c to stop." }
          end
        end

...
After do
  ...
  FileUtils.rm(BUNTO_COMMAND_OUTPUT_FILE) if File.exist?(BUNTO_COMMAND_OUTPUT_FILE)
...
# Like "I have a foo file" but gives a yaml front matter so bunto actually processes it
...
Given /^I have fixture collections$/ do
  FileUtils.cp_r File.join(BUNTO_SOURCE_DIR, "test", "source", "_methods"), source_dir
end
...
When /^I run bunto(.*)$/ do |args|
  status = run_bunto(args)
  if args.include?("--verbose") || ENV['DEBUG']
    puts bunto_run_output
  end
end
...
Then /^I should see "(.*)" in the build output$/ do |text|
  assert_match Regexp.new(text), bunto_run_output
end
